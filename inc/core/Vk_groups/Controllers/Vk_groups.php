<?php
namespace Core\Vk_groups\Controllers;

class Vk_groups extends \CodeIgniter\Controller
{
    ...
    public function index() {
        ...
        $data = [
            ...
            "content" => view('Core\Vk_groups\Views\add', $profiles)
        ];

        return view('Core\Vk_groups\Views\index', $data);
    }
    ...
    public function oauth(){
        ...
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Vk_groups\Views\oauth', [ "config" => $this->config, "oauth_link" => $oauth_link ])
        ];
                    ...
                    foreach ($response->items as $value) {
                        ...
                        if($value->type == "group"){
                            ...
                            if( in_array($value->id, $ids) ){
                                $item = db_get('*', TB_ACCOUNTS, "social_network = 'vk' AND team_id = '{$team_id}' AND pid = '".$value->id."'");

                                if(!$item){
                                    //Check limit number 
                                    check_number_account("vk", "group");
